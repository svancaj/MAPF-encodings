...
INST=ost003d-random-1.scen
instance=$TEST_NAME/instances/scenarios/$INST

for vars in monosat-pass monosat-shift
do
	...
	do
		...
		do
			...
			do
				...
				date
				echo $encoding
				timeout 7200 ./$TEST_NAME/release/MAPF -m $TEST_NAME/instances/maps -s $instance -t $timeout -e $encoding -a 5 -i 5 -q -l 1 -f results_$TASK_NR.res -c $TEST_NAME/tmp.cnf 
			done
		done
	done
done
