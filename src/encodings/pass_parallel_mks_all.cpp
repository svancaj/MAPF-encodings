...
int Pass_parallel_mks_all::Solve(int ags)
{
	...
	while (true)
	{
		PrintSolveDetails(delta);

		// create formula
		auto start = chrono::high_resolution_clock::now();
		CNF = vector<vector<int> >();
		nr_vars = CreateFormula(CNF, delta, time_left);
		nr_clauses = CNF.size();
...
int Pass_parallel_mks_all::CreateFormula(vector<vector<int> >& CNF, int delta, int time_left)
{
